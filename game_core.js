...
var ABILITY_DESCS = {
  ...
  chinatsu: {type:'뒤끝',desc:'아군 무작위 1인에게 보호막을 부여합니다.',gold:'온천 치나츠: 아군 두 명에게 부여',goldDesc:'뒤끝: 아군 무작위 <span style="color:#ffd700;font-weight:700">2인에게</span> 보호막을 부여합니다.'},
  ...
  mari:     {type:'개전',desc:'아군 전체 +2/+1',gold:'아이돌 마리: +3/+3',goldDesc:'개전: 아군 전체 <span style="color:#ffd700;font-weight:700">+3/+3</span>'},
...
function triggerSOC_battlecry_inner(u, mySide, log){
  ...
  else if(id==='izuna'){var atkB=1,hpB=u.golden?1:0;for(var i=0;i<mySide.length;i++){if(mySide[i].alive){mySide[i].atk+=atkB;if(hpB>0)mySide[i].hp+=hpB;}}log.push({cls:'soc',text:'    → '+u.name+': 아군 전체 공격력 +'+atkB+' (연쇄)'});}
  else if(id==='tsukuyo'){var buff=u.golden?2:1;for(var i=0;i<mySide.length;i++){if(mySide[i].alive){mySide[i].atk+=buff;mySide[i].hp+=buff;}}log.push({cls:'soc',text:'    → '+u.name+': 아군 전체 +'+buff+'/+'+buff+' (연쇄)'});}
